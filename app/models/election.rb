class Election < ActiveRecord::Base
  has_many :rolls, -> { includes(:races).order(:name) },
    dependent: :destroy, inverse_of: :election
  has_many :races, -> { includes(:candidates, :roll).order(:name) },
    dependent: :restrict_with_exception, inverse_of: :election do
    def to_csv
      CSV.generate do |csv|
  ...
  scope :purgeable, -> { where { purged_at.eq( nil ) & 
  ...
  def purge!
    self.class.connection.send :delete_sql, "DELETE FROM votes WHERE votes.section_id IN " +
    "(SELECT sections.id FROM sections INNER JOIN ballots ON 
    sections.ballot_id = ballots.id WHERE ballots.election_id = #{id})"
    self.class.connection.send :delete_sql, "DELETE FROM sections WHERE ballot_id IN " +
    "(SELECT id FROM ballots WHERE ballots.election_id = #{id})"
    self.class.connection.send :delete_sql, "DELETE FROM rolls_users WHERE roll_id IN " +
